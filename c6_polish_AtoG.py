...
# Default params
inp_place = _config.OUT_PLACE + 'c_alignment_AtoG/'
NAME = util.get_fn(__file__)
out_place = _config.OUT_PLACE + NAME + '/'
util.ensure_dir_exists(out_place)

exp_design = pd.read_csv(_config.DATA_DIR + 'exp_design.csv')
lib_design = pd.read_csv(_config.DATA_DIR + 'library_AtoG.csv')

global expected_cutsite

names_targets = dict()
for idx, row in lib_design.iterrows():
  names_targets[row['Name (unique)']] = row['Sequence context (56nt)']
...
def save_alignments(data, out_dir, exp):
  ...
  for repair_type in data:
    ...
    aligns, ulmi_counts, qs, ulmis_d, ulmi_qs_d = package
    ...
    with open(out_fn, 'w') as f:
      for alignment in sorted_keys:
        ...
        ulmi_count = ulmi_counts[alignment]
        qf = qsm[alignment]
        f.write('>%s_readct_%s_ulmict\n%s\n%s\n' % (total_count, ulmi_count, alignment, qf))

    ulmi_fn = out_dir + 'ulmis_%s.txt' % (repair_type)
    with open(ulmi_fn, 'w') as f:
      for alignment in sorted_keys:
        total_count = aligns[alignment]
        ulmi_count = ulmi_counts[alignment]
        qf = qsm[alignment]
        # space delimited because ord(' ')-33 = -1 
        ulmis = ' '.join(ulmis_d[alignment])
        ulmi_qs = ' '.join(ulmi_qs_d[alignment])
        f.write('>%s_readct_%s_ulmict\n%s\n%s\n\n' % (total_count, ulmi_count, ulmis, ulmi_qs))
  return

def sort_combine_alignments(aligns):
  store = defaultdict(lambda: 0)
  ulmis = defaultdict(list)
  ulmi_qs = defaultdict(list)
  qs = defaultdict(list)
  for idx, line in enumerate(aligns):
    if idx % 4 == 0:
      header = line
      count_section = header.split('_')[0]
      ulmi = header.split('_')[1]
      ulmi_q = header.split('_')[2]
    ...
    if idx % 4 == 3:
      ...
      ulmis[alignment].append(ulmi)
      ulmi_qs[alignment].append(ulmi_q)
      qs[alignment].append(q)

  ulmi_counts = dict()
  for key in ulmis:
    ulmi_counts[key] = len(set(ulmis[key]))

  return store, ulmi_counts, qs, ulmis, ulmi_qs
...
def gen_qsubs():
  ...
  for condition in exp_design['Name']:
    if 'AtoG' not in condition:
      continue

    command = 'python %s.py %s 0 11999' % (NAME, condition)
    script_id = NAME.split('_')[0]

    # Write shell scripts
    sh_fn = qsubs_dir + 'q_%s_%s.sh' % (script_id, condition)
    with open(sh_fn, 'w') as f:
      f.write('#!/bin/bash\n%s\n' % (command))
    num_scripts += 1

    # Write qsub commands
    qsub_commands.append('qsub -V -l h_rt=8:00:00,h_vmem=2G -wd %s %s &' % (_config.SRC_DIR, sh_fn))

  # Save commands
  commands_fn = qsubs_dir + '_commands_c6_AtoG.sh'
